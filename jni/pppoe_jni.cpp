...
#else // android 7.0 init do not support ctrl.start server with argument
      // so use property to send argument
    sprintf(cmd, "pppoe_setup");
    property_set("net.pppoe.account", c_user);
    property_set("net.pppoe.iface", c_iface);
    property_set("net.pppoe.password", c_password);
#endif
...
static jint startPppoeNative(JNIEnv* env, jobject clazz)
{
        ...
        for (attempt = 150; attempt > 0;  attempt--) {
            ...
            if (!strcmp(prop, "disconnecting") || !strcmp(prop, "disconnected")) {
                LOGE("pppoe fail to start, maybe cause by authentication failed");
                err = -1;
                goto end;
            }
        }
...
{
    return AndroidRuntime::registerNativeMethods(env, "com/android/server/pppoe/PppoeServiceImpl", gPppoeMethods, NELEM(gPppoeMethods) );
}

/* User to register native functions */
extern "C"
jint Java_com_android_server_pppoe_PppoeServiceImpl_registerNatives(JNIEnv* env, jclass clazz) {
   return AndroidRuntime::registerNativeMethods(env, "com/android/server/pppoe/PppoeServiceImpl", gPppoeMethods, NELEM(gPppoeMethods) );
}
